...
export default class GamePage {
  ...
  touchEndCallback = (e) => {
    ...
    this.hit = this.getHitStatus(
      ...
      this.nextBlock,
      blockConf.height / 2 -
        (1 - this.currentBlock.instance.scale.y) * blockConf.height
    );
  ...
  bindTouchEvent() {
    ...
    this.canvas.addEventListener('mousedown', this.touchStartCallback);
    this.canvas.addEventListener('touchend', this.touchEndCallback);
    this.canvas.addEventListener('mouseup', this.touchEndCallback);
  }

  removeTouchEvent() {
    console.log('remove touch event');
    this.canvas.removeEventListener('touchstart', this.touchStartCallback);
    this.canvas.removeEventListener('mousedown', this.touchStartCallback);
    this.canvas.removeEventListener('touchend', this.touchEndCallback);
    this.canvas.removeEventListener('mouseup', this.touchEndCallback);
  }
  ...
  checkBottleHit() {
    if (
      this.checkingHit &&
      this.bottle.obj.position.y <= blockConf.height / 2 + 0.1 &&
  ...
  getHitStatus(bottle, currentBlock, nextBlock, initY) {
    ...
    var time = +(
      (-bottle.velocity.vy +
        Math.sqrt(
          Math.pow(bottle.velocity.vy, 2) - 2 * gameConf.gravity * differenceY
        )) /
      -gameConf.gravity
    ).toFixed(2);
    flyingTime -= time;
    ...
    var destination = [];
    var bottlePosition = new THREE.Vector2(
      bottle.obj.position.x,
      bottle.obj.position.z
    );
    var translate = new THREE.Vector2(this.axis.x, this.axis.z).setLength(
    ...
    if (nextBlock) {
      ...
      if (utils.pointInPolygon(destination, nextPolygon)) {
        ...
        utils.pointInPolygon(
          [destination[0], destination[1] + bottleConf.bodyDepth / 2],
          nextPolygon
        )
      ) {
        result1 = 5;
        ...
        utils.pointInPolygon(
          [destination[0] + bottleConf.bodyDepth / 2, destination[1]],
          nextPolygon
        )
      ) {
        result1 = 3;
