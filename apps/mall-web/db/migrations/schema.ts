...
export const sessions = mysqlTable(
  'sessions',
  {
    ...
    sessionToken: varchar('session_token', { length: 255 }).notNull(), // 会话令牌，唯一
    expiresAt: datetime('expires_at').notNull(), // 过期时间
    ...
    updatedAt: datetime('updated_at')
      ...
      .notNull(), // 更新时间
    ipAddress: varchar('ip_address', { length: 45 }), // IP 地址，支持 IPv4 和 IPv6
    userAgent: text('user_agent'), // 用户代理字符串
  },
  (table) => {
    return {
      id: unique('id').on(table.id),
      userNameUnique: unique('session_token').on(table.sessionToken),
    };
  }
);
